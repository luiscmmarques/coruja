...
- [ ] **Bring-your-own-API-key Google Books.** A Setup field, the key stored locally like everything else, its origins re-added to `connect-src` at build time. For households whose books Open Library does not cover (its non-English children's coverage is thinner).
