...
- **Open Library only.** Google Books was the fallback and was removed rather than fixed: its keyless quota answered `Queries per day: 0` on first real use, so the fallback was a guaranteed 429. Bring-your-own-API-key Google support is a v2 idea — a Setup field, the key stored locally like everything else. A book Open Library does not know is a hand-entered book, which even earns a badge.
